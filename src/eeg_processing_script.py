...
# def ask_input_file_pattern(config, settings):
#     """Ask input_file_patterns with Layout Preview and More Info."""
    
#     # 1. Get Text and Tooltip from settings
#     txt = settings["input_file_patterns", "text"]
...
#     layout = [
#         # 2. Text Label with a clear Help icon/text for the tooltip
#         [
#             sg.Text(txt, font=("Default", 12, "bold"), background_color="white"),
#             sg.Text("(?)", text_color="blue", tooltip=tooltip_text, background_color="white", enable_events=True, key="-HELP-")
#         ],
#         [
#             # 3. Widen the Listbox (size=(45, 12)) to fit the new long names
#             sg.Listbox(
#                 values=settings["input_file_patterns"],
#                 size=(45, 12), 
#                 enable_events=True,
...
#         [
#             sg.Button("Ok", bind_return_key=True),
#             sg.Button("Preview Layout", key="-PREVIEW-"), # New Preview Button
#             sg.Push(background_color="white"), 
#             sg.Button("More info") # New Link Button
#         ],
#     ]

#     window = sg.Window(
#         "Select Acquisition / Montage",
#         layout,
#         modal=True,
#         use_custom_titlebar=True,
#         font=font,
#         location=(100, 100),
#         background_color="white",
#         finalize=True # Needed to bind tooltips properly sometimes
#     )
...
#             # If user closes X, we might want to exit or handle gracefully. 
#             # For now, break to return config (might crash if empty, but consistent with old logic)
#             break
...
#                 sg.popup_error("Please select a layout to preview first.", location=(100, 100), font=font)
#                 continue
            
#             selection = values["_LISTBOX_"][0]
            
#             # Look up the internal montage name from settings
#             # We use the mapping we defined in settings (e.g., "Force Biosemi 64..." -> "biosemi64")
#             montage_name = settings.get(("montage", selection), "n/a")
            
#             if montage_name == "n/a" or montage_name == "MEG":
#                 sg.popup("No standard preview available for 'Native/Auto' or 'MEG' modes.\n(These depend on the specific file content.)", title="Info", location=(100, 100), font=font)
#             else:
#                 try:
#                     # Generate a dummy info object to plot the sensors
#                     # We create a standard montage and plot it
#                     montage = mne.channels.make_standard_montage(montage_name)
#                     # Create a dummy info to allow plotting
#                     # We need to create a dummy raw or info because montage.plot() is deprecated in newer MNE
#                     # But the easiest way in modern MNE is strictly plot_sensors on info
#                     dummy_info = mne.create_info(ch_names=montage.ch_names, sfreq=100, ch_types='eeg')
#                     dummy_info.set_montage(montage)
                    
#                     # Plot 2D sensors
#                     fig = mne.viz.plot_sensors(dummy_info, show_names=True, title=f"Preview: {montage_name}", show=True, block=False)
#                 except Exception as e:
#                     sg.popup_error(f"Could not load preview for {montage_name}:\n{e}", location=(100, 100), font=font)
...
#                     break
#                 else:
#                     sg.popup_error("Please select an option.", location=(100, 100), font=font)
#             except:
#                 sg.popup_error("No valid selection", location=(100, 100), font=font)
#                 window.close()
...
#     # Read data
#     df = pd.read_csv(file_path, sep=r"\s+", skiprows=config.get("header_rows", 1))
    
#     # Ensure we only take the data columns
...
#             "Please check your file format and header_rows setting."
#         ) from e
...
#     Handles .txt files with configurable header structure.
#     Uses generic mne.io.read_raw for all other supported MNE formats.

#     In the settings file you can change:
#     - header_rows: Total number of header rows to skip (default: 1)
#     - channel_names_row: Row number containing channel names (0-based, default: 0 which equals first row)
#     """
#     # Retrieve file path from config to ensure local scope availability
...
#     else:
#         # Use generic MNE reader for all other supported file types (.bdf, .edf, .vhdr, etc.)
#         try:
#             raw = mne.io.read_raw(file_path, preload=True)
#         except Exception as e:
#             # Fallback for unsupported formats
#             sg.popup_error(f"Error reading file {file_path}:\n{e}", location=(100, 100))
#             raise e

#         # Apply specific type picking for formats that require it (legacy logic)
#         if config["file_pattern"] in ["*.fif", "*.cnt"]:
#              raw.pick_types(eeg=True, meg=False, eog=False)
